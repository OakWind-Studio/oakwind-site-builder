...
- Use Magic for **3-4 sections per build** — hero, services/treatment showcase, testimonials, and the CTA break section. These are the highest-impact sections and Magic inspiration elevates them from "good" to "premium." Build simpler sections (contact, footer, trust strip) with OakWind patterns from `references/component-patterns.md`
- **Search queries that return the best results:** `"luxury hero fullscreen"`, `"glass card image hover"`, `"testimonial cards premium"`, `"bento grid glassmorphism"`, `"cta section gallery"`, `"before after comparison slider"`, `"service cards premium hover"`, `"animated stats counter"`, `"split layout image text"`
- After getting inspiration, adapt the patterns to use OakWind's CSS variables, fonts, and color palette — never use the raw output's default styling
...
- Vary CTA break style: use the **CTA Break Rotation System** below — scroll-fill text is only 1 of 7 options
...
#### CTA Break Rotation System

The bottom-half CTA section (the "deep impression" or "decision" moment) must rotate across builds. **Scroll-fill text should appear in no more than 1 in 7 builds.** Pick one per build:

| # | CTA Pattern | Description | Best for |
|---|-------------|-------------|----------|
| 1 | **Split CTA + Gallery Grid** | Text CTA on left, staggered asymmetric photo grid on right (4 images in offset grid areas). Feels editorial and premium. | Med spa, dental, wedding venue, home builder |
| 2 | **Full-width image CTA** | Full-bleed background image with dark overlay, centered headline + CTA. Cinematic and dramatic. | Detailing, restaurant, barber, any visual business |
| 3 | **Stats + CTA bar** | Horizontal row of 3-4 animated stats with a prominent CTA button centered below. Glass morphism background. | Auto repair, home services, attorney — businesses with impressive numbers |
| 4 | **Before/After comparison** | Draggable slider or hover-reveal comparing two states. Use for niches where transformation is the product. | Detailing, pressure washing, dental, med spa |
| 5 | **Testimonial spotlight + CTA** | One massive featured review with oversized quote marks, reviewer photo, and CTA beneath. Scale-reveal on scroll. | Any niche with strong reviews |
| 6 | **Gradient accent bar with animated text** | Full-width accent-colored section with typewriter effect on the headline or character cascade, plus CTA. | Bold niches — barber, restaurant, pressure washing |
| 7 | **Scroll-fill text** | Text that fills with color character-by-character as user scrolls. Dramatic but overused — use sparingly (max 1 in 7 builds). | Reserved for when the tagline is truly exceptional |

**Rotation rule:** Track which CTA pattern was used in the last 3 builds. Never repeat the same pattern in consecutive builds. If the last build used scroll-fill text, the next 6 builds must use a different pattern.
